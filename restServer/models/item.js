...
function Item() 
{
    ...
    this.getAll = function(res) 
    {
        ...
        {
            con.query('select * from localisation', function(err, result) 
            {
                con.release();
                res.send(result);
            });
    ...
    this.get = function(id, res) 
    {
        connection.acquire(function(err, con) 
        {
            con.query('select * from localisation where id = ?', [id], function(err, result) {
                con.release();
                if (err) 
                {
			        console.log(err);
    ...
    this.create = function(item, res) 
    {
        ...
        connection.acquire(function(err, con) 
        {
            con.query('insert into localisation set ?', item, function(err, result) 
            {
                con.release();
                if (err) 
                {
